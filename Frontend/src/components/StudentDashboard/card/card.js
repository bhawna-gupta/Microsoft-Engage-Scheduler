...
const PreferenceItem = ({ choice }) => {
  ...
  const Failure = () => (
    <Dialog className={classes.dialog} open={openFailure}>
      <DialogTitle style={{ textAlign: "center" }}>
        <CancelOutlinedIcon style={{ color: "red", fontSize: "100px" }} />{" "}
        <br />
        Error!
      </DialogTitle>
      <Link
  ...
  const Confirm = () => (
    <Dialog className={classes.dialog} open={openConfirm}>
      <DialogTitle>
        <HelpOutlineIcon style={{ color: "black", fontSize: "100px" }} /> <br />
      ...
      >
        <Button variant="contained" style={{ backgroundColor: "#888888" }}>
          Cancel
        </Button>
      </Link>
    </Dialog>
  ...
  const handleDelete = async () => {
    ...
    await axios({
      method: "DELETE",
      url: `http://localhost:8081/student/dashboard`,
      headers: {
        token: `${token}`,
      },
      data: userObject,
  ...
  const getDatePreference = () => {
    let preferenceDate = choice.date.getDate();
    ...
    <>
      <Card className={classes.card_container}>
        ...
        <CardContent>
          ...
          <div>
            <b>SeatNumber : </b>
            {!choice.seatnumber ? "NA" : choice.seatnumber}
          </div>
          <div>
            <b>Status : </b>
            {choice.status}
          ...
          {/* <Link
            style={{
              textDecoration: "none",
              // display: card.balance > 0 ? "inline" : "none",
            }}
            to={{
              pathname: "/pay",
              state: {
